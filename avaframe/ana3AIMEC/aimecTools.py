...
def readAIMECinputs(avalancheDir, pathDict, defineRunoutArea, dirName="com1DFA"):
    ...
    try:
        message = (
            "There should be exactly one path_aimec.shp file containing the avalanche path in %s/Inputs/LINES/"
            % avalancheDir
        )
    ...
    if defineRunoutArea:
        ...
        try:
            message = (
                "There should be exactly one .shp file containing the split point in %s/Inputs/POINTS/"
                % avalancheDir
            )
    ...
    try:
        assert len(demSource) == 1, (
            "There should be exactly one topography .asc file in %s/Inputs/" % avalancheDir
        )
    except AssertionError:
        raise
...
def fetchReferenceSimNo(avaDir, inputsDF, comModule, cfg, inputDir=""):
    ...
    try:
        ...
        if cfgSetup["varParList"] != "" and (
            any(item in inputsDF.columns.tolist() for item in cfgSetup["varParList"].split("|")) == False
        ):
            message = (
                "No configuration directory found. This is needed for sorting simulation according to "
                "%s" % cfgSetup["varParList"]
            )
            raise (message)
        elif cfgSetup["varParList"] != "" and any(
            item in inputsDF.columns.tolist() for item in cfgSetup["varParList"].split("|")
        ):
            configFound = True
        else:
            configFound = False
    ...
    if configFound and cfgSetup["varParList"] != "":
        ...
        # order simulations
        varParList, inputsDF = cfgHandling.orderSimulations(
            varParList, cfgSetup.getboolean("ascendingOrder"), inputsDF
        )
        colorVariation = True
        ...
        if referenceSimValue != "":
            ...
            log.info(
                (
                    "Reference simulation is the first simulation after reordering according to %s in ascending order"
        ...
        refSimName = inputsDF.loc[refSimRowHash, "simName"]
        message = (
            "No information on how to define the reference was provided, taking simulation %s as reference."
            % refSimName
        )
        log.warning(message)
...
def defineRefOnSimValue(referenceSimValue, varParList, inputsDF):
    ...
    try:
        ...
        log.info(
            (
                "Reference simulation is based on %s = %s - closest value "
...
def defineRefOnSimName(referenceSimName, inputsDF):
    ...
    if len(refSimRowHash) == 0:
        message = "Found no simulation matching the provided referenceSimName = %s." % referenceSimName
        log.error(message)
        raise IndexError(message)
    refSimRowHash, refSimName = checkMultipleSimFound(refSimRowHash, inputsDF)
    log.info(
        "Reference simulation is based on provided simName: %s and corresponds to simulation %s"
...
def checkMultipleSimFound(refSimRowHash, inputsDF, error=False):
    ...
    if len(refSimRowHash) > 1:
        if not error:
            message = (
                "Found multiple simulations (%s) matching the reference criterion, "
...
def resolveResTypeColumn(row, baseResType, layer=""):
    """Resolve base resType to actual DataFrame column for a simulation row

    For single-layer sims: returns baseResType (e.g. 'ppr')
    For multi-layer sims with layer set: returns layer-suffixed column (e.g. 'ppr_l2')
    Returns None if no matching column has data.

    Parameters
    ----------
    row : pandas Series
        a row from inputsDF or resAnalysisDF (one row per simulation with info on
        the simulation, available result files and optional configuration)
    baseResType : str
        base result type name (e.g. 'ppr', 'pfv')
    layer : str
        layer identifier (e.g. 'L2'). Empty string for single-layer.
    ...
    """
    if layer:
        layeredCol = "%s_%s" % (baseResType, layer.lower())
        if layeredCol in row.index and pd.notna(row[layeredCol]):
            return layeredCol
    # single-layer sim or no layer configured
    if baseResType in row.index and pd.notna(row[baseResType]):
        return baseResType
    return None


def computeBaseResTypeList(inputsDF, runoutLayer):
    """Compute base resType names available for all simulations

    Discovers all base resType names from DataFrame columns (stripping _l\\d+ suffixes),
    then checks that every row can resolve each base name via resolveResTypeColumn.
    Returns only base names resolvable for ALL sims.

    Parameters
    ----------
    inputsDF : pandas DataFrame
        simulation dataframe with resType columns (base and/or layer-suffixed)
    runoutLayer : str
        layer identifier (e.g. 'L2'). Empty string for single-layer.
    ...
    """
    # metadata columns to exclude from resType discovery
    metadataCols = {
        "simName",
        "releaseArea",
        "simHash",
    ...
    # discover base resType names by stripping _l\d+ suffixes
    baseNames = set()
    for col in inputsDF.columns:
        if col in metadataCols:
            continue
        baseName = re.sub(r"_l\d+$", "", col)
        baseNames.add(baseName)

    # check per-row resolvability for each base name
    resolvableForAll = []
    for baseName in sorted(baseNames):
        allResolve = True
        for idx in inputsDF.index:
            if resolveResTypeColumn(inputsDF.loc[idx], baseName, layer=runoutLayer) is None:
                allResolve = False
                break
        if allResolve:
            resolvableForAll.append(baseName)

    return resolvableForAll


def checkAIMECinputs(cfgSetup, pathDict, inputsDF):
    ...
    Uses per-row resolution to validate that base resType names are available for
    all simulations (handling both single-layer and multi-layer results).
    Errors if multi-layer data is detected but no runoutLayer is specified.
    ...
    inputsDF: pandas DataFrame
        simulation dataframe with resType columns

    Returns
    -------
    pathDict : dict
        aimec input dictionary updated with:
        - resTypeList: set of base result type names to analyze
        - runoutResType: base runout result type name
        - runoutLayer: layer identifier for downstream resolution
        - displayRunoutResType: display name for plot titles
    """
    # runoutLayer is optional: not needed for single-layer sims, required for multi-layer sims
    runoutLayer = cfgSetup.get("runoutLayer", "")
    runoutResType = cfgSetup["runoutResType"]

    # detect multi-layer data from inputsDF columns
    if not runoutLayer:
        layerColumns = [col for col in inputsDF.columns if re.search(r"_l\d+$", col)]
        if layerColumns:
            message = (
                "Multi-layer result files detected (%s) but no runoutLayer is set. "
    ...
    if cfgSetup["resTypes"] != "":
        ...
        resTypesWanted = list(baseResTypeList)

    # add the runout res type to what we need
    if runoutResType not in resTypesWanted:
        resTypesWanted.append(runoutResType)

    # check that all wanted base types are available for all sims
    diff = [rt for rt in resTypesWanted if rt not in baseResTypeList]
    if diff:
        message = "%s result type should be available for all simulations to analyze." % diff
        log.error(message)
        raise FileNotFoundError(message)

    pathDict["resTypeList"] = sorted(set(resTypesWanted))
    pathDict["runoutResType"] = runoutResType
    pathDict["runoutLayer"] = runoutLayer
    # display name for plot titles: "ppr (if multilayer: L2)" for multi-layer, "ppr" for single-layer
    if runoutLayer:
        pathDict["displayRunoutResType"] = "%s (if multilayer: %s)" % (runoutResType, runoutLayer)
    else:
        pathDict["displayRunoutResType"] = runoutResType
    log.info(
        "Analyzing %s results types. Runout based on %s result type"
...
def computeCellSizeSL(cfgSetup, refCellSize):
    ...
    if cfgSetup["cellSizeSL"] == "":
        ...
        try:
            ...
            message = (
                "cellSizeSL is read from the configuration file but should be a number, you provided: %s"
                % cfgSetup["cellSizeSL"]
            )
            log.error(message)
            raise ValueError(message)
...
def getSArea(rasterTransfo, dem):
    ...
    # project on ney grid
    areaCoef, _ = geoTrans.projectOnGrid(
        rasterTransfo["gridx"] * demCellSize,
        rasterTransfo["gridy"] * demCellSize,
        demAreaCoef,
        csz=demCellSize,
        xllc=xllcenter,
        yllc=yllcenter,
...
def transform(data, name, rasterTransfo, interpMethod, dem=False):
    ...
    log.debug(
        "Data-file: %s - %d raster values transferred - %d out of original raster"
...
def analyzeMass(fnameMass, simRowHash, refSimRowHash, resAnalysisDF, time=None):
    ...
    # analyze mass
    releasedMass, entrainedMass, finalMass, grIndex, grGrad, entMassFlow, totalMass, time = readWrite(
        fnameMass, time
    )
    ...
    log.info(
        "{: <10} {:<10.4f} {:<10.4f}".format(*[resAnalysisDF.loc[simRowHash, "simName"], grIndex, grGrad])
    )
    resAnalysisDF.loc[simRowHash, "relativMassDiff"] = relativMassDiff

    if simRowHash == refSimRowHash:
        resAnalysisDF = pd.concat(
            [resAnalysisDF, pd.DataFrame({"entMassFlowArray": np.nan}, index=resAnalysisDF.index)], axis=1
        ).copy()
        resAnalysisDF["entMassFlowArray"] = resAnalysisDF["entMassFlowArray"].astype(object)
        resAnalysisDF = pd.concat(
            [resAnalysisDF, pd.DataFrame({"totalMassArray": np.nan}, index=resAnalysisDF.index)], axis=1
        ).copy()
...
def computeRunOut(
    cfgSetup, rasterTransfo, resAnalysisDF, transformedRasters, simRowHash, runoutResType
):
    ...
    simRowHash: str
        simulation dataframe hash
    runoutResType: str
        resolved runout result type column name (e.g. "ppr" or "ppr_l1" for multi-layer).
...
def computeRunoutLine(
    ...
    runoutResType: str or None
        resolved runout result type column name (e.g. "ppr" or "ppr_l1" for multi-layer).
        Required when name is empty (simulation runout). Not used when name is set (reference data).
    ...
    if name == "":
        if runoutResType is None:
            raise ValueError("runoutResType is required when computing simulation runout (name is empty)")
    ...
    thresholdValue = cfgSetup.getfloat("thresholdValue")
    runoutLine = {
        "s": np.zeros(rasterTransfo["l"].shape) * np.nan,
        "l": np.zeros(rasterTransfo["l"].shape) * np.nan,
        "x": np.zeros(rasterTransfo["l"].shape) * np.nan,
        "y": np.zeros(rasterTransfo["l"].shape) * np.nan,
...
def analyzeArea(
    ...
    inputs["baseRunoutResType"] = cfgSetup["runoutResType"]
    inputs["displayRunoutResType"] = pathDict["displayRunoutResType"]
    ...
    if areaSum > 0:
        log.debug(
            "{: <15} {:<15.4f} {:<15.4f} {:<15.4f} {:<15.4f}".format(
                *[simName, tp / areaSum, fn / areaSum, fp / areaSum, tn / areaSum]
            )
        )
    if tp + fp == 0:
        log.warning(
            "Simulation %s did not reach the run-out area for threshold %.2f %s"
    ...
    contourDict = outAimec.fetchContourLines(
        rasterTransfo, inputs, cfgSetup.getfloat("thresholdValue"), contourDict
    )
...
def readWrite(fname_ent, time):
    ...
    # check mass balance
    log.info(
        "Total mass change between first and last time step in sim %s is: %.1f kg"
        % (fname_ent.stem, totMassResults[1] - relMass)
    )
    log.info("Total entrained mass in sim %s is: %.1f kg" % (fname_ent.stem, entrainedMass))
...
def findStartOfRunoutArea(dem, rasterTransfo, cfgSetup, splitPoint):
    ...
    if splitPoint != None:
        ...
        rasterTransfo["labelRunout"] = "start of runout area: " + (
            r"$\beta_{%.1f °}$" % rasterTransfo["startOfRunoutAreaAngle"]
        )
    else:
    ...
    log.info(
        "Start of run-out area at the %.2f ° point of coordinates (%.2f, %.2f)"
        ...
        if ref != []:
            for refFile in ref:
                ...
                newLine = pd.DataFrame(
                    [[referenceName, refFile]],
                    columns=["reference_name", "reference_filePath"],
                    index=[referenceName],
                )
...
def computeRunoutPointDiff(resAnalysisDF, refData, simRowHash):
    ...
    for item in ["sRunout", "lRunout"]:
        ...
        log.info(
            "%s Runout diff for %s is %.2f" % (item, resAnalysisDF.loc[simRowHash, "simName"], diffRunout)
        )
...
def addReferenceAnalysisTODF(referenceDF, refFile, refDataDict):
    ...
    for item in ["sRunout", "lRunout", "xRunout", "yRunout"]:
        referenceDF.loc[referenceDF["reference_name"] == refFile.stem, "reference_%s" % item] = refDataDict[
            item
        ]
...
def analyzeDiffsRunoutLines(cfgSetup, runoutLine, refDataTransformed, resAnalysisDF, simRowHash, pathDict):
    ...
    for item in refDataTransformed:
        ...
        if refLine["type"] in ["line", "poly"]:
            ...
            resAnalysisDF.at[simRowHash, "runoutLineDiff_%s" % refLine["type"]] = np.asarray(
                runoutLine["s"] - refLine["s"]
            )
            ...
            diffNoNans = diffAll[
                np.where((np.isnan(refLine["s"]) == False) & (np.isnan(runoutLine["s"]) == False))
            ]
            # check number of points where runoutLine sim has no points but reference Line has points
            runoutLineNoPoints = len(
                np.where(np.isnan(runoutLine["s"][np.where(np.isnan(refLine["s"]) == False)]))[0]
            )
            ...
            refLineNoPoints = len(
                np.where(np.isnan(refLine["s"][np.where(np.isnan(runoutLine["s"]) == False)]))[0]
            )
            ...
            resAnalysisDF.at[simRowHash, "runoutLineDiff_%s_pointsNotFoundInSim" % refLine["type"]] = (
                runoutStr
            )
            resAnalysisDF.at[simRowHash, "runoutLineDiff_%s_pointsNotFoundInRef" % refLine["type"]] = (
                refLineStr
            )
